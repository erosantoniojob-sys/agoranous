...
export const AmbientSoundControl: React.FC = () => {
  ...
  const playNote = (context: AudioContext, output: GainNode, frequency: number, delay = 0) => {
    const oscillator = context.createOscillator()
    const gain = context.createGain()
    oscillator.type = 'triangle'
    oscillator.frequency.setValueAtTime(frequency, context.currentTime + delay)
    gain.gain.setValueAtTime(0.0001, context.currentTime + delay)
    gain.gain.exponentialRampToValueAtTime(0.045, context.currentTime + delay + .025)
  ...
  const start = async () => {
    ...
    droneRef.current = [73.42, 110].map(frequency => {
    ...
    let step = 0
    const phrase = () => {
      playNote(context, master, LYRE_NOTES[step % LYRE_NOTES.length])
      playNote(context, master, LYRE_NOTES[(step + 2) % LYRE_NOTES.length], .48)
